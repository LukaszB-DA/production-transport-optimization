...
Model jest też zaimplementowany w Pythonie (`optimization_production_transport.py`), z użyciem PuLP i dołączonego solvera CBC — trzecie, niezależne sformułowanie i solver, obok wcześniejszej weryfikacji HiGHS.

**Wynik: identyczny zysk 213 750 PLN**, z tymi samymi wielkościami produkcji (P1=800, P2=875) i tym samym rozkładem floty (V3: 49 kursów, V2: 1 kurs do C2, V1 nieużywany) — potwierdzając poprawność sformułowania z Excela po raz kolejny, tym razem przez program całkowitoliczbowy napisany od zera, a nie transkrypcję arkusza.
...
5. Alternatywnie uruchom implementację w Pythonie: `pip install pulp`, następnie `python optimization_production_transport.py`. Oczekiwany wynik: `Status: Optimal`, `Profit: 213750.0`.
